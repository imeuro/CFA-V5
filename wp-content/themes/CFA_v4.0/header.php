			...
			<?php
				if ( class_exists( 'WPGlobus' ) ) {
					...
					foreach( WPGlobus::Config()->enabled_languages as $lang ) {
						if ( $lang == WPGlobus::Config()->language ) { continue; }
						else {
							$hreflang = str_replace( '_', '-', WPGlobus::Config()->locale[$lang] );
							if ($hreflang == 'en-UK') {
								$hreflang = 'en-gb';
							}
							$alt_url = WPGlobus_Utils::localize_current_url( $lang );
						}
					}
					echo '<meta name="language" content="'.strtoupper($cur_lang).'">';
					echo '<link rel="alternate" hreflang="'.$hreflang.'" href="'.$alt_url.'" />';
				}
			?>
  ...
  <div id="menu-coldx" class="hidden">
		<div class="fullcol">
			<span class="text-center">
				...
				<div class="search_submenu submenu"><?php get_search_form(); ?></div>
			</span>
